...
public class BackendErrorReporterService {
    ...
    private Page<ErrorReport> findAll(long userId, ErrorReportStatus[] errorReportStatuses, PageRequest pageRequest) {
        ...
        try {
            ...
            JsonPage<ErrorReport> jsonPage = myApiBackendRequestor.runRequest(
                backendApiUrl,
                params,
                new TypeReference<JsonPage<ErrorReport>>() {
                }
            );
    ...
    public ErrorReport findByLongId(String errorReportId) {
        try {
            return myApiBackendRequestor.runRequest(
                BackendApiUrl.toPrivate("/errorReporter/info"),
                Map.<String, String>of("longId", errorReportId),
                ErrorReport.class
            );
        }
        catch (BackendServiceDownException e) {
