...
public class ConfigPanel extends VerticalLayout {
    ...
    public ConfigPanel(
        ApiBackendRequestor apiBackendRequestor,
        PropertiesServiceImpl configurationService,
        String buttonName,
        Consumer<Properties> action
    ) {
    ...
    private <V> void map(
        Class<V> clazz,
        HasValue<? extends HasValue.ValueChangeEvent<V>, V> property,
        String key,
    ...
    private Component buildCaptchaGroup() {
        return buildGroup(
            ...
            layout -> {
                ...
                map(Boolean.class, enabledCaptcha, CaptchaKeys.CAPTCHA_ENABLED_KEY, () -> false);
                layout.add(enabledCaptcha);

                TextField privateApiKey = TinyComponents.newTextField();
                map(String.class, privateApiKey, CaptchaKeys.CAPTCHA_PRIVATE_KEY, null);
                layout.add(VaadinUIUtil.labeledFill("Private captcha key: ", privateApiKey));

                TextField siteApiKey = TinyComponents.newTextField();
                map(String.class, siteApiKey, CaptchaKeys.CAPTCHA_SITE_KEY, null);
